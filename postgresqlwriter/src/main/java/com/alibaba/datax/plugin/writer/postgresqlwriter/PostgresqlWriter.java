...
public class PostgresqlWriter extends Writer {
	...
	public static class Job extends Writer.Job {
		...
		public void init() {
			...
			if (null != writeMode && !writeMode.startsWith("insert") && !writeMode.startsWith("update")) {
				throw DataXException.asDataXException(DBUtilErrorCode.CONF_ERROR,
					String.format("写入模式(writeMode)配置有误. 因为PostgreSQL不支持配置参数项 writeMode: %s, PostgreSQL仅使用insert sql 或自定义拓展的on conflict do update插入数据. 请检查您的配置并作出修改.", writeMode));
			}
